import numpy as np
from QtMatplotlib import QtMatplotlib
import time




qt_plotter = QtMatplotlib()
# for _ in range(1):
...
# Example of animating a sine wave using QtMatplotlib
if __name__ == "__main__":
    fps = 144  # frames per second
    qt = QtMatplotlib(win_title="Sine Wave Animation")
    ...
    for i in range(20 * fps):  # animate 200 frames
        ...
        plot_num = 0
        qt.plot(x, y, color='c', linewidth=3, live=True, plot_num=plot_num)
        qt.xlabel("X-axis")
        qt.ylabel("Y-axis")
        qt.xlim(0, 2 * np.pi)
        qt.ylim(-1, 1)
